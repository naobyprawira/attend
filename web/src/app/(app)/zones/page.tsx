...
const STATS = [
  { icon: "security", label: "Total Monitored Zones", value: "24", badge: "ACTIVE", badgeBg: "bg-tertiary-fixed", badgeText: "text-tertiary", iconBg: "bg-fuchsia-100", iconColor: "text-primary" },
  { icon: "dangerous", label: "High Security Areas", value: "08", badge: null, iconBg: "bg-red-100", iconColor: "text-error" },
];

const ZONES = [
  { name: "Main Lobby North", sub: "Ground Floor - Section A", icon: "corporate_fare", security: "OPEN", securityColor: "bg-green-100 text-green-700", cameras: ["CAM-01", "CAM-02"], cameraExtra: "+3 more", groups: ["ALL", "VS"], status: "OPERATIONAL", statusOk: true },
  { name: "Data Center 01", sub: "Basement - Restricted", icon: "terminal", security: "HIGH SECURITY", securityColor: "bg-red-100 text-red-700", cameras: ["DC-CAM-01", "DC-CAM-02"], cameraExtra: null, groups: ["ADMIN", "IT"], status: "OPERATIONAL", statusOk: true },
  { name: "Executive Suite", sub: "Level 12 - West Wing", icon: "meeting_room", security: "RESTRICTED", securityColor: "bg-orange-100 text-orange-700", cameras: ["EXEC-04"], cameraExtra: null, groups: ["EXEC", "+2"], status: "MAINTENANCE", statusOk: false },
  { name: "Employee Lounge", sub: "Level 04 - Commons", icon: "restaurant", security: "OPEN", securityColor: "bg-green-100 text-green-700", cameras: ["LOUNGE-01"], cameraExtra: null, groups: ["STAFF"], status: "OPERATIONAL", statusOk: true },
];
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-surface min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant/10 col-span-1 sm:col-span-2 relative overflow-hidden group">
          <div className="relative z-10 flex flex-col justify-between h-full">
            <div>
              <p className="text-on-surface-variant text-sm font-medium mb-1">AI Scan Efficiency</p>
          ...
          <div className="absolute -right-4 -bottom-4 text-surface-container opacity-10 group-hover:scale-110 transition-transform duration-700">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant/5">
              {ZONES.map((z) => (
                <tr key={z.name} className="hover:bg-surface-container transition-colors group">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-on-surface">{z.name}</p>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {z.cameras.map((c) => (
                        <span key={c} className="px-2 py-0.5 bg-secondary-fixed text-on-secondary-fixed text-[10px] font-bold rounded">{c}</span>
                      ))}
                  ...
                  <td className="px-6 py-5">
                    <div className="flex -space-x-2">
                      {z.groups.map((g) => (
                        <div key={g} className="h-6 w-6 rounded-full bg-primary-container text-white border-2 border-surface-container-lowest flex items-center justify-center text-[8px] font-bold">
                          {g}
                        </div>
                      ))}
                    </div>
