  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 bg-surface min-h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Topology Diagram */}
        <div className="lg:col-span-2 bg-surface-variant rounded-xl border border-outline-variant/5 p-4 sm:p-6 lg:p-8">
          ...
          <div className="relative w-full" style={{ minHeight: 380 }}>
            ...
            {NODES.map((node) => (
              ...
              >
                ...
                <span className="text-[10px] font-bold text-on-surface mt-2 whitespace-nowrap">
                  {node.label}
                </span>
        ...
        <div className="bg-surface-variant rounded-xl border border-outline-variant/5 overflow-hidden flex flex-col">
          ...
          <div className="divide-y divide-outline-variant/5 flex-1">
            {INCIDENTS.map((inc, i) => (
              <div key={i} className="px-5 py-4 hover:bg-surface-container-high transition-colors">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h5 className="text-sm font-bold text-on-surface">{inc.title}</h5>
                </div>
                <p className="text-xs text-on-surface-variant ml-6">{inc.detail}</p>
                <p className="text-[10px] text-on-surface-variant mt-1.5 ml-6">{inc.time}</p>
              </div>
            ))}
          </div>
...
function StatBox({ label, value, icon }: { label: string; value: string; icon: string }) {
  return (
    <div className="bg-surface-variant p-5 rounded-xl border border-outline-variant/10 relative overflow-hidden group">
