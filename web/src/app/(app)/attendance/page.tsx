...
const statusConfig: Record<Status, { label: string; bg: string; text: string; dot: string }> = {
  ...
  late: { label: "Late", bg: "bg-amber-100", text: "text-amber-700", dot: "bg-amber-600" },
  absent: { label: "Absent", bg: "bg-error-container/30", text: "text-error", dot: "bg-error" },
  "not-yet": { label: "Not Yet", bg: "bg-surface-container", text: "text-secondary", dot: "bg-secondary/50" },
};
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 lg:space-y-12">
      ...
      <section className="space-y-8">
        ...
        {/* Stat Cards */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-0.5 rounded-2xl overflow-hidden shadow-[0_20px_50px_-12px_rgba(27,28,29,0.08)] bg-outline-variant/10">
          <div className="bg-surface-container-lowest p-4 sm:p-6 flex items-center justify-between">
            <div>
          ...
          <div className="bg-surface-container-lowest p-4 sm:p-6 flex items-center justify-between">
            ...
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-amber-50 flex items-center justify-center text-amber-600">
          ...
          <div className="bg-surface-container-lowest p-4 sm:p-6 flex items-center justify-between">
            ...
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-surface-container-high flex items-center justify-center text-secondary">
      ...
      <section className="flex flex-wrap items-center gap-4 sm:gap-6">
        <div className="flex items-center gap-3 bg-surface-container-low px-4 py-2 rounded-xl">
          <span className="text-[10px] font-bold uppercase text-on-surface-variant tracking-widest">Dept:</span>
          <select
            value={department}
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
        {filtered.map((emp) => {
          ...
          return (
            <Link
              ...
              className="bg-surface-container-lowest p-6 rounded-2xl shadow-[0_20px_50px_-12px_rgba(27,28,29,0.08)] hover:-translate-y-1 transition-transform duration-300 group block"
            >
              <div className="flex items-start justify-between mb-6">
                <div className={`w-16 h-16 rounded-xl bg-surface-container flex items-center justify-center ${emp.status === "absent" || emp.status === "not-yet" ? "opacity-50" : ""}`}>
        ...
        {/* Enroll Card */}
        <div className="bg-surface-container-lowest p-6 rounded-2xl border-2 border-dashed border-outline-variant/30 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-surface-container-low transition-colors min-h-[220px]">
